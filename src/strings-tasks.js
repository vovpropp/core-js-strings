...
function removeFirstOccurrences(str, value) {
  ...
  const str1 = str.slice(0, str.indexOf(value));
  const str2 = str.slice(str.indexOf(value) + value.length);
  return str1 + str2;
}
/**
...
function removeLastOccurrences(str, value) {
  if (!str.includes(value)) return str;
  const str1 = str.slice(0, str.lastIndexOf(value));
  const str2 = str.slice(str.lastIndexOf(value) + value.length);
  return str1 + str2;
}
...
function findLongestWord(sentence) {
  const s = sentence.split(' ').sort((a, b) => b.length - a.length)[0];
...
function encodeToRot13(str) {
  const a1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const a2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let r = '';
  for (let i = 0; i < str.length; i += 1) {
    if (a1.includes(str[i])) {
      r += a2[a1.indexOf(str[i])];
    } else {
      r += str[i];
    }
  }
